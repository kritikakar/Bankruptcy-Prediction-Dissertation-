...
opt_inputs <- function(x.train, y.train) {
  ...
  for (alpha_val in alpha_values) {
    ...
    lambda_values <- c(lambda_values, best_lambda)
    cv_results[[paste("alpha_", alpha_val, sep = "")]] <- cv_elnet
  }
  
  best_index <- which.min(sapply(cv_results, function(cv) min(cv$cvm)))
  best_alpha <- alpha_values[best_index]
  best_lambda <- lambda_values[best_index]
  
  best_combination <- data.frame(alpha = best_alpha, lambda = best_lambda)
...
best_elnet31 <- opt_inputs(x.train, y.train)

elnet_31 <- glmnet(x.train, y.train, family = "binomial", alpha = best_elnet31$alpha, lambda = best_elnet31$lambda)
...
best_elnet13 <- opt_inputs(x.train.subset, y.train)

elnet_13 <- glmnet(x.train.subset, y.train, family = "binomial", alpha = best_elnet13$alpha, lambda = best_elnet13$lambda)
...
test50 <- anti_join(final_data, train50)
...
best_elnet31_50 <- opt_inputs(x_train_50, y_train_50)

elnet31_50 <- glmnet(x_train_50, y_train_50, family = "binomial", alpha = best_elnet31_50$alpha, lambda = best_elnet31_50$lambda)

# choosing the best coefficients

coef(elnet31_50)

selected_vars_50 <- c("Attr6", "Attr13", "Attr15", "Attr21","Attr23", "Attr24", "Attr26", "Attr29", "Attr33", "Attr34", "Attr35", "Attr36", "Attr38", "Attr39", "Attr41", "Attr45", "Attr47", "Attr58", "Attr61")
...
best_elnet19_50 <- opt_inputs(x_train_50, y_train_50)

elnet19_50 <- glmnet(x_train_50_subset, y_train_50, family = "binomial", alpha = best_elnet19_50$alpha, lambda = best_elnet19_50$lambda)

evaluation2(elnet19_50, as.matrix(x_test_50_subset), as.vector(y_test_50))
...
rf18_50 <- randomForest(data = train50, class ~ Attr26 + Attr24 + Attr13 + Attr6 + Attr5 + Attr38 + Attr58 + Attr23 + Attr21 + Attr45 + Attr34 + Attr39 + Attr25 + Attr41 + Attr15 + Attr35 + Attr29 + Attr17, ntree = 500)
...
evaluation3(rf31_50, test50)

evaluation3(rf15_50, test50)

evaluation3(rf18_50, test50)
...
predictions_rf18_50 <- predict(rf18_50, test50, type = "class")
...
predictions_elnet19_50 <- predict(elnet19_50, x_test_50_subset, type = "response")
predictions_elnet19_50 <- ifelse(predictions_elnet19_50 > 0.5, 1, 0)

num.ytest_50 <- as.numeric(ytest_50)

roc_rf18_50 <- roc(num.ytest_50, as.numeric(predictions_rf18_50))
...
roc_elnet19_50 <- roc(as.vector(test50$class), predictions_elnet19_50[, 1])
...
plot(roc_elnet19_50, col = "cyan3", add = TRUE)
...
evaluation3(rf18_50, test50)
evaluation3(rf18_50, train50)

# may be overfitted

# fitting with top 5 variables

rf5_50 <- randomForest(data = train50, class ~ Attr26 + Attr24 + Attr13 + Attr6 + Attr5, ntree = 500)
...
test25 <- anti_join(final_data, train25)
...
best_elnet31_25 <- opt_inputs(x_train_25, y_train_25)

elnet31_25 <- glmnet(x_train_25, y_train_25, family = "binomial", alpha = best_elnet31_25$alpha, lambda = best_elnet31_25$lambda)

coef(elnet31_25)

selected_vars_25 <- c("Attr6", "Attr15", "Attr24", "Attr26", "Attr28", "Attr29", "Attr32", "Attr33", "Attr34", "Attr36", "Attr38", "Attr39", "Attr41", "Attr45", "Attr47", "Attr48", "Attr55", "Attr59", "Attr61")
...
best_elnet19_25 <- opt_inputs(x_train_25_subset, y_train_25)

elnet19_25 <- glmnet(x_train_25_subset, y_train_25, family = "binomial", alpha = best_elnet19_25$alpha, lambda = best_elnet19_25$lambda)
...
rf15_25 <- randomForest(data = train25, class ~ Attr24 + Attr26 + Attr13 + Attr58 + Attr38 + Attr23 + Attr21 + Attr5 + Attr34 + Attr45 + Attr41 + Attr15 + Attr6 + Attr35 + Attr25, ntree = 500, importance = TRUE)

# evaluating models

evaluation(logitstep_25, test25)
evaluation(logit_int_25, test25)
evaluation2(elnet19_25, x_test_25_subset, y_test_25)
...
predictions_elnet19_25 <- predict(elnet19_25, x_test_25_subset, type = "response")
predictions_elnet19_25 <- ifelse(predictions_elnet19_25 > 0.5, 1, 0)
...
roc_elnet19_25 <- roc(as.vector(test25$class), predictions_elnet19_25[, 1])
...
plot(roc_elnet19_25, col = "cyan3", add = TRUE)
